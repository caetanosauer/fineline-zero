...
class warehouse_man_impl : public table_man_t<warehouse_t>
{
...
public:
    ...
    warehouse_man_impl(warehouse_t* aWarehouseDesc)
        : table_man_t<warehouse_t>(aWarehouseDesc)
    { }

    ~warehouse_man_impl() { }

    // --- access specific tuples  --- //
    w_rc_t wh_index_probe(Database* db,
                          warehouse_tuple* ptuple,
                          const int w_id);

    w_rc_t wh_index_probe_forupdate(Database* db,
                                    warehouse_tuple* ptuple,
                                    const int w_id);


    // --- update a retrieved tuple --- //
    w_rc_t wh_update_ytd(Database* db,
                         warehouse_tuple* ptuple,
...
class district_man_impl : public table_man_t<district_t>
{
...
public:
    ...
    district_man_impl(district_t* aDistrictDesc)
        : table_man_t<district_t>(aDistrictDesc)
    { }

    ~district_man_impl() { }

    // --- access specific tuples --- //
    w_rc_t dist_index_probe(Database* db,
                            district_tuple* ptuple,
                            const int w_id,
                            const int d_id);

    w_rc_t dist_index_probe_forupdate(Database* db,
                                      district_tuple* ptuple,
                                      const int w_id,
                                      const int d_id);
    ...
    // --- update a retrieved tuple --- //
    w_rc_t dist_update_ytd(Database* db,
                           district_tuple* ptuple,
                           const double h_amount);


    w_rc_t dist_update_next_o_id(Database* db,
                                 district_tuple* ptuple,
                                 const int  next_o_id);
...
class customer_man_impl : public table_man_t<customer_t>
{
...
public:
    ...
    customer_man_impl(customer_t* aCustomerDesc)
        : table_man_t<customer_t>(aCustomerDesc)
    { }

    ~customer_man_impl() { }

    // --- access tuples with iterator --- //
    w_rc_t cust_get_iter_by_index(Database* db,
    ...
    // --- access specific tuples --- //
    w_rc_t cust_index_probe(Database* db,
                            customer_tuple* ptuple,
                            const int w_id,
                            const int d_id,
                            const int c_id);

    w_rc_t cust_index_probe_by_name(Database* db,
                                    const char* idx_name,
                                    customer_tuple* ptuple,
                                    const int w_id,
                                    const int d_id,
    ...
    w_rc_t cust_index_probe_forupdate(Database* db,
                                      customer_tuple* ptuple,
                                      const int w_id,
                                      const int d_id,
                                      const int c_id);
    ...
    // --- update a retrieved tuple --- //
    w_rc_t cust_update_tuple(Database* db,
                             customer_tuple* ptuple,
                             const tpcc_customer_tuple& acustomer,
                             const char* adata1 = NULL,
                             const char* adata2 = NULL);

    w_rc_t cust_update_discount_balance(Database* db,
                                        customer_tuple* ptuple,
...
public:
    ...
    history_man_impl(history_t* aHistoryDesc)
        : table_man_t<history_t>(aHistoryDesc)
    { }
...
class new_order_man_impl : public table_man_t<new_order_t>
{
...
public:
    ...
    new_order_man_impl(new_order_t* aNewOrderDesc)
        : table_man_t<new_order_t>(aNewOrderDesc)
    {
    }
    ...
    // --- access tuples with iterator --- //
    w_rc_t no_get_iter_by_index(Database* db,
    ...
    // --- update a retrieved tuple --- //
    w_rc_t no_delete_by_index(Database* db,
                              new_order_tuple* ptuple,
                              const int w_id,
                              const int d_id,
                              const int o_id);
...
class order_man_impl : public table_man_t<order_t>
{
...
public:
    ...
    order_man_impl(order_t* aOrderDesc)
        : table_man_t<order_t>(aOrderDesc)
    {
    }
    ...
    // --- access tuples with iterator --- //
    w_rc_t ord_get_iter_by_index(Database* db,
    ...
    // --- update a retrieved tuple --- //
    w_rc_t ord_update_carrier_by_index(Database* db,
                                       order_tuple* ptuple,
...
class order_line_man_impl : public table_man_t<order_line_t>
{
...
public:
    ...
    order_line_man_impl(order_line_t* aOrderLineDesc)
        : table_man_t<order_line_t>(aOrderLineDesc)
    {
    }
    ...
    w_rc_t ol_get_range_iter_by_index(Database* db,
    ...
    w_rc_t ol_get_probe_iter_by_index(Database* db,
                                      order_line_table_iter* &iter,
                                      order_line_tuple* ptuple,
                                      rep_row_t &replow,
                                      rep_row_t &rephigh,
...
class item_man_impl : public table_man_t<item_t>
{
...
public:
    ...
    w_rc_t it_index_probe(Database* ddb,
                          item_tuple* ptuple,
                          const int i_id);

    w_rc_t it_index_probe_forupdate(Database* db,
                                    item_tuple* ptuple,
                                    const int i_id);
...
class stock_man_impl : public table_man_t<stock_t>
{
...
public:
    ...
    stock_man_impl(stock_t* aStockDesc)
        : table_man_t<stock_t>(aStockDesc)
    {
    }
    ...
    /* --- access the table --- */
    w_rc_t st_index_probe(Database* db,
                          stock_tuple* ptuple,
                          const int w_id,
                          const int i_id);

    w_rc_t st_index_probe_forupdate(Database* db,
                                    stock_tuple* ptuple,
                                    const int w_id,
                                    const int i_id);

    /* --- update a retrieved tuple --- */
    w_rc_t st_update_tuple(Database* db,
                           stock_tuple* ptuple,
